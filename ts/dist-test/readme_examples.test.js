...
const __1 = require("..");
const SDK_NAME = 'NavalnyarchiveSDK';
...
const TEST_SEED = { "entity": { "daily_post": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+NavalnyarchiveSDK\b/.test(code) ||
        /\bNavalnyarchiveSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+NavalnyarchiveSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/NavalnyarchiveSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
