...
Navalnyarchive API client, generated from the OpenAPI spec.

This API provides access to the content from navalny.com, including blogs, social media, projects, and photographs associated with Alexei Navalny.

Learn more about Navalnyarchive API at [navalny.com](https://navalny.com).

This is an unofficial SDK for the Navalnyarchive public API, generated by Voxgig with [`@voxgig/sdkgen`](https://github.com/voxgig/sdkgen). It is not affiliated with, endorsed by, or sponsored by the upstream API provider.
...
This SDK exposes the API as a small set of **semantic entities** — DailyPost — that you
...
const items = await client.DailyPost().list()
```
...
```go
client := sdk.Test()
result, err := client.DailyPost(nil).List(
    nil, nil,
)
```
...
```bash
cd go-mcp && go build -o navalnyarchive-mcp .
```
...
```json
{
  "mcpServers": {
    "navalnyarchive": {
      "command": "/abs/path/to/navalnyarchive-mcp"
    }
  }
}
```
...
| --- | --- | --- |
| **DailyPost** | The DailyPost entity (list). | `/ru/daily-posts/today/` |
...
# List all dailyposts (returns a list, raises on error)
dailyposts = client.DailyPost().list()
...
// List all dailyposts (returns an array; throws on error)
$dailyposts = $client->DailyPost()->list();
print_r($dailyposts);
```
...
```go
import sdk "github.com/voxgig-sdk/navalnyarchive-sdk/go"
...
# List all dailyposts (returns an Array; raises on error)
dailyposts = client.DailyPost.list
puts dailyposts
```
...
Generated from the Navalnyarchive API OpenAPI spec by [@voxgig/sdkgen](https://github.com/voxgig/sdkgen).
