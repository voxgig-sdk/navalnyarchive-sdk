...
boru-driven command-line client **and** interactive REPL for the Navalnyarchive
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/navalnyarchive-cli)
make build

# 2. See usage (words, entities, env vars)
./navalnyarchive-cli --help

# 3. Provide credentials once, via the environment
export NAVALNYARCHIVE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./navalnyarchive-cli list daily_post

# 5. Override the API base URL for a single call
NAVALNYARCHIVE_BASE=https://api.example.com ./navalnyarchive-cli list daily_post

# 6. No arguments -> interactive REPL
./navalnyarchive-cli
navalnyarchive> list daily_post
navalnyarchive> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/navalnyarchive-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/navalnyarchive-cli list daily_post
   ```
...
Run with no arguments to open a REPL (prompt `navalnyarchive>`). Each line is
...
- `<entity>` is a bareword, auto-quoted as an boru atom (e.g. `daily_post`).
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/navalnyarchive-cli`. |
...
(`./navalnyarchive-cli <expr>`) and interactively (the REPL), and expressions compose the
