// Command Navalnyarchive-cli is a CLI + REPL for the Navalnyarchive SDK,
...
const prompt = "navalnyarchive"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "daily_post"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      NAVALNYARCHIVE_APIKEY (api key), NAVALNYARCHIVE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: NAVALNYARCHIVE_APIKEY carries the API key and
	// NAVALNYARCHIVE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("NAVALNYARCHIVE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("NAVALNYARCHIVE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewNavalnyarchiveSDK(opts)
