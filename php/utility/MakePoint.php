...
// Navalnyarchive SDK utility: make_point

require_once __DIR__ . '/../core/Helpers.php';

class NavalnyarchiveMakePoint
{
    public static function call(NavalnyarchiveContext $ctx): array
    {
        if (isset($ctx->out['point'])) {
            ...
            if ($ctx->out['point'] instanceof NavalnyarchiveError) {
        ...
        if (count($op->points) === 1) {
            ...
            foreach ($op->points as $p) {
                $select_def = NavalnyarchiveHelpers::to_map(\Voxgig\Struct\Struct::getprop($p, 'select'));
                $found = true;
            ...
            if ($reqselector) {
                $req_action = \Voxgig\Struct\Struct::getprop($reqselector, '$action');
                if ($req_action && $point) {
                    $point_select = NavalnyarchiveHelpers::to_map(\Voxgig\Struct\Struct::getprop($point, 'select'));
