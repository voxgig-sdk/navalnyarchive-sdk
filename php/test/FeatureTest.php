...
// Navalnyarchive SDK feature test
//
...
class FtCtrl extends NavalnyarchiveControl
{
...
class FtClient
{
    ...
    private NavalnyarchiveUtility $utility;

    public function __construct(NavalnyarchiveUtility $utility, array $options)
    {
    ...
    public function get_utility(): NavalnyarchiveUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (NavalnyarchiveContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public NavalnyarchiveContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (NavalnyarchiveContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = NavalnyarchiveConfig::make_feature($name);
    ...
    private static function buildUrl(NavalnyarchiveSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof NavalnyarchiveError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new NavalnyarchiveResponse($fetched);
        }
    ...
    private function fail(NavalnyarchiveContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (NavalnyarchiveContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
