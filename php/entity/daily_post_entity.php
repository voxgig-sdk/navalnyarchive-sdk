...
// Navalnyarchive SDK DailyPost entity
...
class DailyPostEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DailyPostEntity($this->_client, $opts);
    }

    /**
     * @param DailyPost|array $args DailyPost data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = NavalnyarchiveHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return DailyPost|array The current DailyPost data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of DailyPost fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = NavalnyarchiveHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of DailyPost fields).
     */
    ...
    /**
     * List DailyPost items matching the given filter.
     *
     * @param DailyPostListMatch|array|null $reqmatch Match filter (any subset
     *   of DailyPost fields) as an assoc-array; DailyPostListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return DailyPost[]|array A list of DailyPost items as assoc-arrays at
     *   the SDK boundary; throws NavalnyarchiveError on failure (item-5 convention).
     */
