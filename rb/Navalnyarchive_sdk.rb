...
require_relative 'Navalnyarchive_types'
...
class NavalnyarchiveSDK
  ...
  def initialize(options = {})
    ...
    feature_opts = NavalnyarchiveHelpers.to_map(VoxgigStruct.getprop(@options, "feature"))
    if feature_opts
      featureorder = VoxgigStruct.getpath(@options, "__derived__.featureorder")
      if featureorder.is_a?(Array)
        featureorder.each do |fname|
          fopts = NavalnyarchiveHelpers.to_map(feature_opts[fname])
          if fopts && fopts["active"] == true
            utility.feature_add.call(@_rootctx, NavalnyarchiveFeatures.make_feature(fname))
          end
        end
      end
    end
  ...
  def get_utility
    NavalnyarchiveUtility.copy(@_utility)
  end
  ...
  def prepare(fetchargs = {})
    ...
    ctrl = NavalnyarchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "ctrl")) || {}
    ...
    params = NavalnyarchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "params")) || {}
    query = NavalnyarchiveHelpers.to_map(VoxgigStruct.getprop(fetchargs, "query")) || {}
    ...
    ctx.spec = NavalnyarchiveSpec.new({
  ...
  def raw_request(fetchargs = {})
    ...
    rescue NavalnyarchiveError => err
    ...
    if fetched.is_a?(Hash)
      status = NavalnyarchiveHelpers.to_int(VoxgigStruct.getprop(fetched, "status"))
  ...
  def graphql(query, variables = nil, ctrl = nil)
    ...
    if errors.is_a?(Array) && !errors.empty?
      ...
      res["ok"] = false
      res["err"] = NavalnyarchiveError.new(
        "graphql_error", "NavalnyarchiveSDK: graphql: #{msg}")
      res["graphql"] = errors
    end
  ...
  # Canonical facade: client.DailyPost.list / client.DailyPost.load({ "id" => ... })
  def DailyPost(data = nil)
    require_relative 'entity/daily_post_entity'
    DailyPostEntity.new(self, data)
  end
  ...
  def self.test(testopts = nil, sdkopts = nil)
    ...
    sdk = NavalnyarchiveSDK.new(sdkopts)
    sdk.mode = "test"
    sdk
  end
end
