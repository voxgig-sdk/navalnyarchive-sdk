...
class DailyPostEntity
  ...
  def make
    opts = @_entopts.dup
    DailyPostEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = NavalnyarchiveHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [DailyPost, Hash] the current DailyPost data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = NavalnyarchiveHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of DailyPost fields)
  def match_get
  ...
  #
  # @param reqmatch [DailyPostListMatch, Hash, nil] match filter (any subset of
  #   DailyPost fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<DailyPost>, Array] the matching DailyPost items; raises NavalnyarchiveError on failure
