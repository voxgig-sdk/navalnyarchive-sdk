...
local DailyPostEntity = {}
DailyPostEntity.__index = DailyPostEntity


function DailyPostEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DailyPostEntity)
  self._name = "daily_post"
  self._client = client
...
function DailyPostEntity:get_name()
  return self._name
end


function DailyPostEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DailyPostEntity.new(self._client, opts)
end
...
function DailyPostEntity:mark_deleted()
  self._deleted = true
end


function DailyPostEntity:deleted()
  return true == self._deleted
end


function DailyPostEntity:data_set(args)
  if args ~= nil then
...
function DailyPostEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DailyPostEntity:match_set(args)
  if args ~= nil then
...
function DailyPostEntity:match_get()
...
function DailyPostEntity:stream(action, args, callopts)
...
---@param reqmatch DailyPostListMatch
---@param ctrl? table
---@return DailyPost[]
---@return string? err
function DailyPostEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function DailyPostEntity:_run_op(ctx, post_done)
  local utility = self._utility
