...
from navalnyarchive_sdk.utility import register

# Load features
from navalnyarchive_sdk.feature.base_feature import NavalnyarchiveBaseFeature
from navalnyarchive_sdk.features import _has_feature, _make_feature
...
class NavalnyarchiveSDK:
    ...
    def get_utility(self):
        return NavalnyarchiveUtility.copy(self._utility)
    ...
    def graphql(self, query, variables=None, ctrl=None):
        ...
        if isinstance(errors, list) and 0 < len(errors):
            ...
            res["ok"] = False
            res["err"] = Exception("NavalnyarchiveSDK: graphql: " + str(msg))
            res["graphql"] = errors

        return res


    def DailyPost(self, data=None) -> "DailyPostEntity":
        """Entity factory: client.DailyPost().list() / client.DailyPost().load({"id": ...})."""
        from navalnyarchive_sdk.entity.daily_post_entity import DailyPostEntity
        return DailyPostEntity(self, data)



    @classmethod
    def test(cls, testopts=None, sdkopts=None) -> "NavalnyarchiveSDK":
        if sdkopts is None:
            sdkopts = {}
