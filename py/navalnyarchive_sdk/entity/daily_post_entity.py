...
from navalnyarchive_sdk.navalnyarchive_types import (
    DailyPost,
    DailyPostListMatch,
)
...
class DailyPostEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return DailyPostEntity(self._client, opts)
    ...
    def list(self, reqmatch=None, ctrl=None) -> list[DailyPost]:
        utility = self._utility
        # reqmatch is optional: an omitted match lists all records. Treat None
        # as an empty match so client.DailyPost().list() works with no args.
        if reqmatch is None:
            reqmatch = {}
