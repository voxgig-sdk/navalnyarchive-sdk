# Navalnyarchive SDK test runner
...
class NavalnyarchiveTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            NavalnyarchiveTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = NavalnyarchiveTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = NavalnyarchiveTestRunner.getenv("NAVALNYARCHIVE_TEST_LIVE")
        override = NavalnyarchiveTestRunner.getenv("NAVALNYARCHIVE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = NavalnyarchiveTestRunner.getenv(key)
        ...
        explain = NavalnyarchiveTestRunner.getenv("NAVALNYARCHIVE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["NAVALNYARCHIVE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if NavalnyarchiveTestRunner._test_control is not None:
            return NavalnyarchiveTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                NavalnyarchiveTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            NavalnyarchiveTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = NavalnyarchiveTestRunner.load_test_control()
...
def load_env_local():
    NavalnyarchiveTestRunner.load_env_local()


def env_override(m):
    return NavalnyarchiveTestRunner.env_override(m)


def entity_data(v):
    return NavalnyarchiveTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return NavalnyarchiveTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return NavalnyarchiveTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return NavalnyarchiveTestRunner.load_test_control()


def live_client_options():
    return NavalnyarchiveTestRunner.live_client_options()


def live_delay_ms():
    return NavalnyarchiveTestRunner.live_delay_ms()
