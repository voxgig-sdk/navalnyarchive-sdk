...
[MCP](https://modelcontextprotocol.io) server exposing the Navalnyarchive SDK as
two agent tools — `navalnyarchive_list` and `navalnyarchive_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/navalnyarchive-mcp)
make build

# 2. Provide credentials via the environment
export NAVALNYARCHIVE_APIKEY=sk_live_xxx

# 3a. Install into Claude Code over stdio (most common)
claude mcp add --scope user navalnyarchive \
  -- /absolute/path/to/navalnyarchive-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./navalnyarchive-mcp -transport http -addr :8080
```

Tool-call arguments (what an agent sends):

```jsonc
// navalnyarchive_list: first page of records
{ "entity": "daily_post" }
{ "entity": "daily_post", "query": { } }

// navalnyarchive_load: one record by id
{ "entity": "daily_post", "query": { "id": 1 } }
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/navalnyarchive-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user navalnyarchive \
     -- "$PWD"/dist/*/navalnyarchive-mcp -transport stdio
   ```

4. **Restart Claude Code.** The `navalnyarchive_list` and `navalnyarchive_load` tools now appear
   in new sessions. Ask the agent to *"list daily_post using navalnyarchive"*
   and it calls `navalnyarchive_list` with `{"entity":"daily_post"}`.
...
```sh
export NAVALNYARCHIVE_APIKEY=sk_live_xxx            # API key
export NAVALNYARCHIVE_BASE=https://api.example.com  # optional: override the API base URL
```
...
### Call the `navalnyarchive_list` tool
...
### Call the `navalnyarchive_load` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `navalnyarchive_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `navalnyarchive_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
| Variable | Purpose |
|----------|---------|
| `NAVALNYARCHIVE_APIKEY` | API key sent with every request. |
| `NAVALNYARCHIVE_BASE` | Optional override of the API base URL. |
...
```sh
./navalnyarchive-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"navalnyarchive_load","arguments":{"entity":"daily_post","query":{"id":1}}}}'
```
