...
module NavalnyarchiveUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = NavalnyarchiveResponse.new({})
    ...
    if fetch_err
      ...
      response = NavalnyarchiveResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = NavalnyarchiveResponse.new(fetched)
    else
