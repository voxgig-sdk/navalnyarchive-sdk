...
// Navalnyarchive SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class NavalnyarchiveTestFeature extends NavalnyarchiveBaseFeature
{
    ...
    public function init(NavalnyarchiveContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (NavalnyarchiveContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = NavalnyarchiveParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (NavalnyarchiveContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(NavalnyarchiveContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = NavalnyarchiveParam::call($ctx, $k);
