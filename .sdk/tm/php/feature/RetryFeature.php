...
// Navalnyarchive SDK retry feature
...
class NavalnyarchiveRetryFeature extends NavalnyarchiveBaseFeature
{
    ...
    public function init(NavalnyarchiveContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (NavalnyarchiveContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(NavalnyarchiveContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(NavalnyarchiveContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
