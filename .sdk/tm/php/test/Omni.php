...
//    only - a typed NavalnyarchiveContext there would make every ctx
...
final class NavalnyarchiveOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new NavalnyarchiveContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new NavalnyarchiveSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new NavalnyarchiveResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new NavalnyarchiveError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new NavalnyarchiveResponse($clean['response']);
    ...
    public static function err_from_map($m): ?NavalnyarchiveError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new NavalnyarchiveError(is_string($code) ? $code : '', $msg);
    }
