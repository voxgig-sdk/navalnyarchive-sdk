...
import (
	...
	omni "github.com/voxgig-sdk/navalnyarchive-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.NavalnyarchiveError); is {
				out := map[string]any{
					"name":    "NavalnyarchiveError",
...
func omniCtx(arg any, client *sdk.NavalnyarchiveSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.NavalnyarchiveSDK); is {
			client = live
